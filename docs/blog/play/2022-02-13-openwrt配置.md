...
- `domainneeded`: Tells dnsmasq never to forward queries for plain names, without dots or domain parts, to upstream nameservers. If the name is not known from /etc/hosts or DHCP then a “not found” answer is returned
- `localise_queries`: Choose IP address to match the incoming interface if multiple addresses are assigned to a host name in `/etc/hosts`.
- `expandhosts`: Add the local domain part to names found in `/etc/hosts`

- `local`: Look up DNS entries for this domain from `/etc/hosts`.
- `domain`: DNS domain handed out to DHCP clients

- `localservice '0'`: 允许除局域网内设备访问
...
config dnsmasq
        ...
        option rebind_protection '0'  # 1，解析到 private 地址时将其丢弃
        option port '53'
        list listen_address '0.0.0.0'
        list listen_address '::'
        option confdir '/etc/dnsmasq.d'
        option localservice '0'    # 允许非局域网内设备访问，即使 bind_adder
        option noresolv '1'
        option localuse '1'        # Use _dnsmasq_ as a local system resolver.
