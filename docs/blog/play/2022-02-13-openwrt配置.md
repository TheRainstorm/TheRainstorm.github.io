...
![image.png](https://imagebed.yfycloud.site/2025/12/3402ca11bb6022c1e3a6c73b19966830.png)
...
**新版本配置全变了，以下内容废弃**，请参考：[[OpenWrt Wiki] Zerotier](https://openwrt.org/docs/guide-user/services/vpn/zerotier)
...
- 修改/etc/config/zerotier 内容（存在样本配置），下面代码为使用命令行配置方法
   ...
   uci add_list zerotier.openwrt_network.join='159924d630e57be5'
   uci set zerotier.openwrt_network.enabled='1'
   uci set zerotier.openwrt_network.config_path='/etc/zerotier'
   uci commit zerotier
   ```

1. 启动 zerotier（这步之后已经可以从 ZeroTier Central 中看到路由器连上了）
...
1. 防火墙
   ...
   ![image-20210916172318383](https://imagebed.yfycloud.site/2025/12/6c76665a0686bbd23f6c403a5c24569f.png)

2. **ZeroTier Central 配置路由转发**
   ...
   ![image-20210707233753989](https://imagebed.yfycloud.site/2025/12/5ddfc401fc42d399484f2ea4b279048f.png)
...
```
root@xdr6088 ➜  ~ opkg files zerotier
...
```
root@tr3000:~# opkg files zerotier
...
1. 依赖，对于22.03以后的版本，实测不需要安装其它内核模块。官方文档写对于openwrt21.02 ，需要安装nat模块
...
1. 设置nft规则，从wg接口进入的流量出去时需要进行src nat。
...
1. 允许ULA地址路由。虽然我们上面开启了SNAT是不会把私有地址发送到公网的，但是路由器对于ULA地址仍然默认是不会进行路由转发的，需要手动开启。（这里要注意的是防火墙postrouting规则是在进行路由选择之后生效的，参考[netfilter-packet-flow](https://upload.wikimedia.org/wikipedia/commons/3/37/Netfilter-packet-flow.svg)）
...
[[OpenWrt Wiki] Tunneling interface protocols](https://openwrt.org/docs/guide-user/network/tunneling_interface_protocols#protocol_wireguard_wireguard_vpn)

- fwmark：配合策略路由，专门而对 wg 流量设置路由

```
from all fwmark 0xca6c lookup wifi_table
```

- nohostroute：默认为 0，会在 main 路由表中添加一条到 endpoint 的路由。在我需要控制 wg 走哪条路由时，可以将其设置为 0。
...
[gSpotx2f/luci-app-temp-status: Temperature sensors data for the LuCI status page (OpenWrt webUI)](https://github.com/gSpotx2f/luci-app-temp-status)
...
Adapter: ISA adapter
temp1:        +46.0°C  (high = +120.0°C, crit = +110.0°C)

mdio_bus:01-mdio-1
Adapter: MDIO adapter
temp1:        +55.5°C  (high = +120.0°C)
...
设备可以通过 DHCPv6 PD 获取一个 前缀，或者 SLAAC 获取一个 64 位前缀。本质都是你拥有了这一段前缀的路由权。

小于 64 的前缀，可以 ra，dhcpv6 server 啥的。

大于 64 的前缀，

- 就只能 dhcpv6 了，
  - android 不吃 dhcpv6
  - openwrt 界面里没有设置  dhcpv6 前缀的方式。只有这个接口从 PD 获得一个 64 前缀后，才能设置 RA server，dhcpv6 server 啥的。**可以使用 adguardhome。**
- 或者使用 ULA + NAT66。
...
- `noresolv 1`: 不使用上游 dns，而只是用配置文件中设置的上游 dns。Don't read upstream servers from `/etc/resolv.conf` which is linked to `resolvfile` by default
...
- `option rebind_protection '0'`: 允许解析出 lan 地址，**(2024-02-03) 现在rebind_protection是默认开启的。因此需要显示设置为0**
- `localservice`：只允许来自本地网络的 DNS 查询，如果希望路由器作为别的网络的 dns 服务器，需要设置为 0。Accept DNS queries only from hosts whose address is on a local subnet, ie a subnet for which an interface exists on the server.
...
#### 总是使用上游 dns（noresolve 失效）

mi4a 上，即使设置了 noresolv，但是仍然会根据 `/tmp/resolv.conf.d/resolv.conf.auto` 进行解析。
...
解决办法为，在网络接口中 uncheck **Use DNS servers advertised by peer**，或者配置文件中添加  `option peerdns 0`
...
若自定义 DNS 重写（如将 `home.lan` 指向 `192.168.1.52`）但未显式禁用 IPv6 响应，客户端查询 AAAA 记录时可能收到空响应（`NXDOMAIN` 或 `::`）

这是正常的，windows ping 时在 v4 不通的情况下才会去尝试 v6
...
[看图说话，论sqm qos对家用环境的有效性与必要性-OPENWRT专版-恩山无线论坛](https://www.right.com.cn/forum/thread-4093940-1-1.html)
[OpenWrt sqm qos标记dscp优先级&配置ipv6 “端口转发”和docker+macvlan获取ipv6地址&wireguard组网 - 知乎](https://zhuanlan.zhihu.com/p/683091761)

[[OpenWrt Wiki] SQM (Smart Queue Management)](https://openwrt.org/docs/guide-user/network/traffic-shaping/sqm)
