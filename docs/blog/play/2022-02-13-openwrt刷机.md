...
![image-20221029213022447](https://imagebed.yfycloud.site/2025/12/73bd4747fb2efd24c0c32705b18acf87.png)
...
![image.png](https://imagebed.yfycloud.site/2025/12/717dc1c1eba161c5fb3f3f28bf2a5cb0.png)

![image.png](https://imagebed.yfycloud.site/2025/12/486901223b4990a9cf239e5308647524.png)
...
- 第一次自己刷机红米AC2100[[记录红米AC2100折腾]]，一张精华图
  ...
  ![image-20210622145337241](https://imagebed.yfycloud.site/2025/12/d6b1f5fb7bb01c670e5c411bdf7b5104.png)
...
```
   ...
   ![image-20221030210955796](https://imagebed.yfycloud.site/2025/12/2849adf13c2aa58670f522cb1da270ab.png)
...
![](https://imagebed.yfycloud.site/2025/12/e24b0bf62f073d5cbc93b00f508112a7.jpg)
...
![](https://imagebed.yfycloud.site/2025/12/aaa5140585a7417a323a910e3d0adcd6.png)
...
![](https://imagebed.yfycloud.site/2025/12/33803a5a0a3754c1e6f79795e7de6466.png)
...
![](https://imagebed.yfycloud.site/2025/12/824349c28a4aef10d7eee06e68aec134.png)

![](https://imagebed.yfycloud.site/2025/12/e79e3e1f410669bd0544125b7a7c0769.png)
...
机器拆机，通过串口刷了 hanwckf 的uboot。带 webui，长按 reset 后上传系统镜像即可

immortalwrt 23.04 有 openwrt uboot layout（文件中包含 ubootmod）和 custom layout。选择 custom layout 的，下载 sysupgrade 即可。
24.04 的没有后缀了（应该只有 openwrt layout了），需要安装 openwrt 官方教程重新刷官方 uboot，才可以用。
...
有 128MB 和 256MB flash 的版本，我买的不幸是 128MB 的。

#### 刷 openwrt

官方提供了过渡固件（openwrt 固件，禁用了修改 fip 分区）：[OpenWrt Software Download – Cudy](https://www.cudy.com/blogs/faq/openwrt-software-download)
20251124 update：SN2543（43 week）后制造的使用新的 flash，openwrt 在 24.10.5 才支持，使用旧版的openwrt中间固件会变砖

官方中间固件放在了google 网盘：[Cudy Intermediary OpenWRT Firmware - Google 云端硬盘](https://drive.google.com/drive/folders/1BKVarlwlNxf7uJUtRhuMGUqeCa5KpMnj)
下载最新的即可
...
亲测从 immortalwrt 可以传 sysupgrade 镜像升级到 openwrt

24.10.1 immortalwrt 有 "Cudy TR3000 v1" 和 "Cudy TR3000 v1 (OpenWrt U-Boot layout)" 两种镜像，而 openwrt 只有默认一种。二者从文件来看，immortalwrt 前一种反而和 openwrt 一样，包含 Kernel 和 sysupgrade 两个文件。"Cudy TR3000 v1 (OpenWrt U-Boot layout)" 包含 uboot 的 fip 文件。
...
preloader 我印象中使用 7981串口万能恢复时也用到了

#### 刷大分区

[Cudy TR3000 的 Ubootmod 布局 - 面向开发者 - OpenWrt 论坛 --- Ubootmod layout for Cudy TR3000 - For Developers - OpenWrt Forum](https://forum.openwrt.org/t/ubootmod-layout-for-cudy-tr3000/232058/8)

[mediatek: add ubootmod layout for cudy tr3000 v1 by 1715173329 · Pull Request #17712 · openwrt/openwrt](https://github.com/openwrt/openwrt/pull/17712)
[[openwrt-24.10] mediatek: add ubootmod layout for cudy tr3000 v1 by 1715173329 · Pull Request #18945 · openwrt/openwrt](https://github.com/openwrt/openwrt/pull/18945)

已经刷了小分区 64MB 官方 openwrt 镜像了。确实是可以更换改版 uboot 的。
...
目前 (20250824) 只有 snapshot 版本编译了 ubootmod（显示为 openwrt U-Boot layout），不知道

- 256mb 的是有的 tr3000 是 256MB 闪存？(是的)
![image.png](https://imagebed.yfycloud.site/2025/12/43303bafc8b337169d8e3a0515cd6a61.png)

20250114 update：24.10.5 已经有了
![image.png](https://imagebed.yfycloud.site/2026/01/49c7d271b1266d250c81a2c459f9e7a4.png)


步骤

- stock
- 升级 cudy 提供的中间固件（intermediate）（本质是 openwrt initramfs？）
- 升级 sysupgrade 固件
- （以上为原本刷 openwrt 步骤，有了 ssh 后就可以刷 uboot-mod）
- ssh 安装 kmod-mtd-rw
- 刷 BL2 和 FIP
- 断电 reset开机， tftp 刷 initramfs recovery 固件
  - firmware selector 自定义的固件，没有提供。可以直接使用官方的
- 刷 sysupgrade

经验

- 我用 openwrt 的官方系统，直接刷了 immortalwrt 的 BL2 和 FIP，最后恢复到 immortalwrt 是可以的
- 我用immortalwrt 网站自定义了固件，生成了 uboot 和 sysupgrade.itb，但是没有 initramfs-recovery.itb。用官方的 initramfs 也可以
- cudy tr3000 断电 reset，直接就是白灯常亮，可以 ping 通
- tftp 恢复时，必须看清楚需要设置哪个静态 ip，否则可能一点反应都没有。比如cudy 的恢复教程中让设置 192.168.1.88，但是 uboot layout 这个 pr 写的是 192.168.1.254
- tftp 恢复时需要的文件名，可以在软件中看到。本次操作需要的文件名就是不带版本号的，和下载的文件名不一样。
...
reg 表示起始位置是 0x5c0000，大小是 0x7a40000（122.25MB）
...
ubinfo -a 可以看到所有卷（volum）使用

| 卷 ID | 卷名称 (Name)  | 大小 (Size)  | 用途说明                             |
| ---- | ----------- | ---------- | -------------------------------- |
| 0    | kernel      | 3.9 MiB    | 存放 Linux 内核。                     |
| 1    | fit         | 39.9 MiB   | 存放只读的根文件系统 (/rom 就是从这里来的)。       |
| 2    | recovery    | 13.3 MiB   | 恢复系统，一个“隐形”的备份系统。                |
| 3    | ubootenv    | 1.0 MiB    | U-Boot 引导程序的环境变量。                |
| 4    | ubootenv2   | 1.0 MiB    | U-Boot 环境变量的备份。                  |
| 5    | rootfs_data | 55.8 MiB   | 可写的 overlay 分区 (/overlay 就是它)。   |
| -    | UBI 保留块     | ~2.4 MiB   | 20 个保留块 * 124 KiB/块，用于坏块管理和磨损均衡。 |
| 总计   |             | ~117.3 MiB |                                  |

#### 256MB 版本 tr3000

这次咸鱼 178 拍卖买了一个 256MB 全新仅拆箱的版本，同时期 128MB 全新最低为 152 

256MB 版本的好处是不用担心刷 openwrt 改版 uboot 的问题，直接使用 stock uboot，就有 190MB 的空间

```
root@cudy-tr3000-v1-256:~# df -h
Filesystem                Size      Used Available Use% Mounted on
/dev/root                 4.8M      4.8M         0 100% /rom
tmpfs                   242.9M    188.0K    242.7M   0% /tmp
/dev/ubi0_2             191.0M     52.0K    186.2M   0% /overlay
overlayfs:/overlay      191.0M     52.0K    186.2M   0% /
tmpfs                   512.0K         0    512.0K   0% /dev
```

0xe600000 表示 ubi 有 230MB
```
partition@5c0000 {
        label = "ubi";
        compatible = "linux,ubi";
...
- 重置 stock 系统，输入背面密码，进入 webui
- 固件升级为官方提供的中间固件（256MB 的版本，和普通版本是两个文件）
- 升级为 openwrt 24.10.5 官方 sysupgrade 固件
...
原厂系统
![](https://imagebed.yfycloud.site/2025/12/58ac2fc50f257810121e5b3cdc0de056.png)
支持的功能
![](https://imagebed.yfycloud.site/2025/12/576c6bec7339246310ea2653d8d01cbc.png)

从咸鱼上买的，路由器系统里可以看到 1 月份的日志。
![](https://imagebed.yfycloud.site/2025/12/007a7aeb273a69f13f3030925277b0a9.png)

uboot 控制台
![](https://imagebed.yfycloud.site/2025/12/fd1467768f496b9f01f5ed77c078e159.png)

![](https://imagebed.yfycloud.site/2025/12/8d7eed0a25ffebf3e72ea0dc8999b4a5.png)

webui 刷机时串口输出
![](https://imagebed.yfycloud.site/2025/12/7178d275807f10c8131269ee82468ddf.png)
...
恐怖的 CPU 占用
![](https://imagebed.yfycloud.site/2025/12/e87c84df2f1a713841fb6c1b59ef70e6.png)
