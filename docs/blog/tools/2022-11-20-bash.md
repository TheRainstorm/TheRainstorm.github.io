...
- 未定义时替换
![](https://imagebed.yfycloud.site/2025/12/f17da23f413bb7a1e5e0c1ce53f9e1cc.png)

- 定义时替换
![](https://imagebed.yfycloud.site/2025/12/15c6af5d36dc9d06d2fc26034d261256.png)
用途
...
在Shell脚本中，所有变量本质上都是字符串类型，但数值和字符串的比较操作符不同，这决定了判断逻辑的差异。
...
- ❌ 错误：`if [ $foo = 0 ]`（若 `foo=` 未定义，展开后变成 `[ = 0 ]`，语法错误）
- ✅ 修正：始终用双引号包裹变量（`[ "$foo" = 0 ]`）
...
- 单引号和双引号在 EOF 中不需要转义（普通的 echo `"\""` 是需要转义的）
- `$` 由 EOF 是否加引号决定
