...
_注意：ffmpeg 很多选项，其关于 input/output 的位置的不同，会产生完全不同的效果_
...
```
ffmpeg -ss [开始时间] -i [输入视频文件] -to [结束时间] -c copy [输出视频文件]
```

- `-ss [开始时间]`: 指定剪切的起始时间点。**把它放在 `-i` 前面**可以利用关键帧进行快速定位，速度会非常快。
- `-i [输入视频文件]`: 指定你要处理的原始视频文件，例如 `my_movie.mp4`。
- `-to [结束时间]`: 指定剪切的结束时间点。
- `-c copy`: 这是**最关键**的参数。`c` 是 `codec`（编解码器）的缩写，`copy` 表示直接复制，不对视频流（`-c:v copy`）和音频流（`-c:a copy`）进行任何重新编码。
- `[输出视频文件]`: 你希望保存的新视频文件的名称，例如 `clip.mp4`。

- 时间格式
  - `HH:MM:SS.ms` (时:分:秒.毫秒)，例如 `00:01:23.500`。
  - `SS` (秒)，例如 `83` 就代表 1 分 23 秒。
...
![FPS vs crf, preset](https://imagebed.yfycloud.site/2025/12/7c50d18a437fb36bd6431a883d20e2f2.)

![VMAF vs CRF, preset](https://imagebed.yfycloud.site/2025/12/41bbeb72442428fe76b0603aa9f4cd8d.)
...
![](https://imagebed.yfycloud.site/2025/12/a367264dc24676dd4c09cef62c2c9bdd.png)
