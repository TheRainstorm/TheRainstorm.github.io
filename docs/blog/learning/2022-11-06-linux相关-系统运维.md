...
- systemd
  ...
  - 配置文件位于`/etc/systemd/network`，使用类似于 ini 格式
- netplan
...
停止并禁用传统网络服务（如 `NetworkManager` 或 `ifupdown`），避免冲突
...
```
vim /etc/systemd/network/10-static-eth0.network
...
```
auto vmbr0.10
iface vmbr0.10 inet6 auto

iface vmbr0.10 inet static
        address 192.168.35.253/24
        post-up ip ro r default via 192.168.35.1 dev vmbr0.10 metric 100
        post-down ip ro d default metric 100

auto vmbr0.20
iface vmbr0.20 inet6 auto

iface vmbr0.20 inet static
        address 192.168.36.253/24
        post-up ip ro r default via 192.168.36.1 dev vmbr0.20 metric 200
        post-down ip ro d default metric 200
```
...
```
iface enx001122334455 inet manual # 原USB网卡名称
hwaddress ether aa:bb:cc:dd:ee:ff # 新增此行，替换为实际MAC
```
...
```
/etc/systemd/network/10-usb-net.link

```

```
[Match]  
MACAddress=aa:bb:cc:dd:ee:ff  # USB网卡MAC  
[Link]  
Name=usb0  # 自定义名称
```

```
iface usb0 inet manual # 替换为自定义名称
```
...
- 名称彻底固定：自定义名称（如`usb0`) 不依赖物理位置
- 长期稳定：适合生产环境

### Udev规则法（传统方案，需谨慎）

潜在冲突：若与Systemd命名规则共存，可能导致设备重复命名
...
```
dmesg --console-off   # 或简写 sudo dmesg -D
dmesg --console-on    # 或 sudo dmesg -E


dmesg -n 1             # 设置控制台仅显示紧急消息
dmesg -n 7           # 恢复默认级别（通常为 7）
```

dmesg -D 等价于 dmesg -n 1，但是 dmesg -D 会可以通过 -E 恢复回原本的日志级别。

直接修改内核参数 `printk` 的第一个值（控制台日志阈值）。值为 `1` 时仅显示 `KERN_EMERG` 消息
...
![image.png](https://imagebed.yfycloud.site/2025/12/37905686bb5220953a590492904288b1.png)
...
- CA
  - 证书颁发机构，本质是一个私钥+ CA证书（公钥）
  - 服务器要想获得证书，需要像 CA 申请，CA 签名产生证书。
  - 自签名证书，就是 CA 是自己的，需要将 CA 证书添加到系统的受信任的证书
- 根 CA 证书
  - CA 支持多级结构，安装到系统的都是根 CA 证书
  - 如果有多级 CA，服务器需要提供完整的证书链
- 证书签名请求（CSR）
  - 由于 CA 和需要签名的服务器通常不在一台机器，因此使用 CSR 作为中介
    - 服务器私钥生成 CSR
    - CA 服务器通过 CA 签名 CSR，生成证书（crt, pem）
- crt
  - 是证书文件的扩展名，可以是PEM或DER格式。
- pem
  - 使用Base64编码的文本文件，通常以-----BEGIN CERTIFICATE-----开头，以-----END CERTIFICATE-----结尾。

![image.png](https://imagebed.yfycloud.site/2025/12/46de015920963264c8871bf4f8f1be14.png)

将多个证书（如根证书、中间证书）合并成一个证书链（如果是 root CA 直接签名，则不需要）

```
cat root-cert.pem intermediate-cert.pem server-cert.pem > full-chain.pem
```
...
CN = My Private Root CA  # Common Name
...
basicConstraints = critical, CA:TRUE  # 标记为根 CA
keyUsage = critical, keyCertSign, cRLSign  # 允许签发证书和吊销列表
```
...
openssl genrsa -out rootCA.key 4096 # 无密码保护

# 生成根 CA 自签名证书（有效期 100 年）
openssl req -x509 -new -key rootCA.key -sha256 -days 36500 -out rootCA.crt -config ca.cnf
```

### 将根 CA 证书安装到 Windows 受信任根存储

**双击 `rootCA.crt`** → 点击 **安装证书**

要选择添加到 这台计算机（非用户），**受信任的根文件存储（不要自动）**，否则之后会报 CA 错误 ERR_CERT_AUTHORITY_INVALID
...
[alt_names]
DNS.1 = yourdomain.com  # 必须包含域名
DNS.2 = *.yourdomain.com  # 支持通配符
DNS.3 = localhost        # 本地测试需添加
```
...
openssl req -new \
  ...
  -addext "subjectAltName = DNS:yourdomain.com, DNS:localhost" \
  -CA /etc/pve/pve-root-ca.pem \
  -CAkey /etc/pve/priv/pve-root-ca.key \
  -noenc -newkey rsa:2048 \
  -keyout server.key -out server.pem
```

- ca 和 server 私钥位于同一机器

### 常见错误

#### ERR_CERT_AUTHORITY_INVALID

这表示浏览器无法信任颁发给网站的 SSL/TLS 证书的**证书颁发机构 (CA)**

windows 系统已经安装自签名证书，但是浏览器还是报 ERR_CERT_AUTHORITY_INVALID，需要安装到受信任的根证书办法机构

![image.png](https://imagebed.yfycloud.site/2025/12/b6568f58775f413d7e4f78d84864c527.png)

#### ERR_CERT_COMMON_NAME_INVALID

subject

**使用者” (Subject)**，Common Name (CN)
...
手动删除所有 dependency problems 的包。删除一遍后，继续apt 随便 install 一个包，看还有没有报错的包，继续删除。

```
sudo dpkg --force-all --purge  libc6-dev-x32 gcc-9-multilib libclang-common-10-dev lib32ubsan1 lib32stdc++6 lib32quadmath0 lib32itm1 lib32gomp1 lib32gcc-s1 lib32atomic1 lib32asan5 g++-9-multilib g++-multilib lib32gcc-9-dev libx32stdc++-9-dev g++-9-multilib gcc-multilib lib32stdc++-9-dev lib32stdc++-9-dev libx32gcc-9-dev 
```
...
![](https://imagebed.yfycloud.site/2025/12/91c1d9235ea5287bf084afc7920c3293.png)
