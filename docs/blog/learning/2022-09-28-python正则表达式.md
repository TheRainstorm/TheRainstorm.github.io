...
finiter  # findall 的字典替代
list_of_dicts = [match.groupdict() for match in pattern.finditer(text)]
...
默认情况下，元字符 `.` 可以匹配除了换行符 `\n` 之外的任何字符。当你需要一个跨越多行的模式时（例如，匹配一个以 `START` 开始、以 `END` 结束的完整文本块），这个默认行为就会成为障碍。
...
re.search(r"BEGIN(.*)END", text, re.DOTALL)  # 简写 re.S
```

#### 让 `^` 和 `$` 匹配每行的开头和结尾

默认情况下，锚点 `^` 只匹配整个字符串的开头，而 `$` 只匹配整个字符串的结尾。如果你想在多行文本中查找以特定模式开头或结尾的**行**，这个默认行为就不够用了。
...
matches_multiline = re.findall(r"^ID: \d+", data, re.MULTILINE)  # 简写 re.M
```
