...
### 其它子网无法 ping 通 jcq_q30p vxlan

jcq_q30p 上现在的路由如下。访问目的地址 192.168.35.0/24 的 lan（之后简称 lan35） 会走 vxlan，其余默认路由走 wg0。
...
如果 lan36 想要访问 jcg_q30p 下的 lan35，比如 op2 下的设备 ping jcg_q30p。由于 lan35本质位于 op1 下，所以 op2 下的设备 icmp request 是可以送到 jcg_q30p 的，通过 vxlan 接口。但是由于 op2 位于 lan36，jcq reply 的包，路由会走 wg0。而 op1 和 jcq 间 wg0 是只允许互相发送 10.0.31.x/32 的包的。因此 reply 无法通过 wg0 接口。

解决问题的方法是添加**源进源出**规则，可以认为是网络中的**光路可逆原理**了。包既然可以从一个接口送达，那么也可以从一个接口返回。
